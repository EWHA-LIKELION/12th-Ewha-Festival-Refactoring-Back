...
class GuestBookUserView(views.APIView):
    ...
    def post(self, request, pk, format=None):
        ...
        serializer = GuestBookSerializer(data=request.data, context={
                                         'request': request, 'booth_id': booth.id})
...
class GuestBookDeleteView(views.APIView):
    ...
    def delete(self, request, pk, guestbook_id):
        ...
        guestbook_entry = get_object_or_404(
            Guestbook, booth=booth, id=guestbook_id)
...
class BoothScrapView(views.APIView):
    ...
    def delete(self, request, pk):
        ...
        boothScrap = get_object_or_404(
            Booth_scrap, user=request.user.id, booth=pk)
        booth = get_object_or_404(Booth, pk=pk)
...
class MenuScrapView(views.APIView):
    ...
    def delete(self, request, pk, menu_id):
        ...
        menuScrap = get_object_or_404(
            Menu_scrap, user=request.user.id, menu=menu_id)
        menu = get_object_or_404(Menu, pk=menu_id)
